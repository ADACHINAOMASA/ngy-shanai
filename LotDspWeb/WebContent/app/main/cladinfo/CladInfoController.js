...
(function () {
    ...
    var controllerName = 'CladInfoController';
    ...
    var newController = function (
        ...
        $scope.action = {
        	...
        	init:function(){
        		...
        		//画面別データ取得
        		$scope.action.setStaffCladInfosData(JSON.parse(localStorage.getItem(lotInfoStorage)).cladInfoBean);
        	}
	    	//共通データの設定
        	...
        	//データの設定
    		,setStaffCladInfosData:function(data){
    			$scope.cladInfo = data;
    			$scope.cladBoxInfos = [];
    			if ($scope.cladInfo.cladInfoBox != null) {
        			if ($scope.cladInfo.cladInfoBox.length > 0) {
        		        for(var i = 0; i < $scope.cladInfo.cladInfoBox.length; i++) {
        	    			$scope.cladBoxInfos.push($scope.cladInfo.cladInfoBox[i]);
        		        }
            		}
    			}
     		}
    		...
    		,search:function(){
				...
				ModalService.showProcessing(SearchService.searchAction(param), {message:'処理中・・・'}).then(function(data) {
    				if(data.errorFlg){
    	        		...
    	        		//画面別データ取得
    	        		$scope.action.setStaffCladInfosData(data.cladInfoBean);
    				}
				});
    		}
    		//移動
