(function () {
	...
	var name = 'HinmokubetsuChokurohiListModalController';
	
    var injectParams = [
        '$scope', '$uibModalInstance', '$q','input', 'multiSelect', 
        'uiGridConstants', '$timeout', 'NisGridUtil', 'SysConst', 
        'i18nService', 'ModalService', 'LoginService', 'HinmokubetsuChokurohiService',
        'messages', 'header'
    ];
    ...
    var ListModalController = function (
		...
		// 初期値 ------------------------------------------------------------------------------ ↓
		// タイトル
		$scope.title = SysConst.hinmokubetsuChokurohi.ichiran.title;
		// 入力情報
		$scope.input = input || {};
		//ヘッダー情報
		$scope.header = header || {};
		...
		$scope.action = {
			// 検索
			search : function() {
				ModalService.loading(HinmokubetsuChokurohiService.list($scope.input))
				.then(function(data){
					$scope.records = data;
