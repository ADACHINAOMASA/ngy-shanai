    ...
    var newController = function (
        ...
        $scope.action = {
        	...
        	init:function(){
        		...
        		//画面別データ取得
        		$scope.action.setStaffManufactureInfosData(JSON.parse(localStorage.getItem(lotInfoStorage)).staffManufactureBean);
        	}
	    	//共通データの設定
        	...
        	//データの設定
    		,setStaffManufactureInfosData:function(data){
    			$scope.staffManufactureInfo = data;
    		}
    		//検索
    		,search:function(){
		    	//メッセージエリアのクリア
				...
				ModalService.showProcessing(SearchService.searchAction(param), {message:'処理中・・・'}).then(function(data) {
    				if(data.errorFlg){
    	        		...
    	        		//画面別データ取得
    	        		$scope.action.setStaffManufactureInfosData(data.staffManufactureBean);
    				}
				});
    		}
    		//移動
