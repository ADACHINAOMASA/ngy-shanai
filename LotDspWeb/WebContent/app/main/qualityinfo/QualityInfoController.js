    ...
    var newController = function (
        ...
        $scope.action = {
        	...
        	init:function(){
        		...
        		//画面別データ取得
        		$scope.action.getStaffQualityInfosData();
        	}
	    	//共通データの設定
        	...
        	//データの設定
    		,getStaffQualityInfosData:function(){
    			var info = JSON.parse(localStorage.getItem(lotInfoStorage)).staffQualityBean;
    			if (info.staffQualityBoxInfos.length > 0) {
    		        for(var i = 0; i < info.staffQualityBoxInfos.length; i++) {
    	    			$scope.staffQualityBoxInfos.push(info.staffQualityBoxInfos[i]);
    		        }
        		}
