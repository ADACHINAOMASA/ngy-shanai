/*
 *	Nippon Light Metal Co.Ltd Medis 2004
 *	NumberUtil.java
 *	履歴：	2003.11.25	Yukihito-Takahashi	新規作成
 *
 */
...
public class NumberUtil {
	...
	public static BigDecimal parse(String val) {
		...
		try {
			decimal = new BigDecimal(trimComma(val.trim()));
		}
	 ...
	 * 小数点以下の右ゼロをトリムする
	 */
	public static String trimRightZero(String val) {
		if (val == null) {
			return null;
		}
		return val.replaceAll("()\\.0+$|(\\..+?)0+$", "$2");
	}

	/**
	 * nvlをしてからaddする
	 */
	public static BigDecimal addNvl(BigDecimal lhs, BigDecimal rhs) {
		return nvl(lhs).add(nvl(rhs));
	}

	/**
	 * nvlをしてからsubtractする
	 */
	public static BigDecimal subtractNvl(BigDecimal lhs, BigDecimal rhs) {
		return nvl(lhs).subtract(nvl(rhs));
	}
